...
Library_Common()
{
	...
	truclient_step("2", "Function SelectCasesApplyFilter", "snapshot=Common_2.inf");
	{
		...
		truclient_step("2.1", "Navigate to https://manage-case.perf...tform.hmcts.net/", "snapshot=Common_2.1.inf");
		...
		truclient_step("2.4", "Select Sifil from Case type listbox", "snapshot=Common_2.4.inf");
		truclient_step("2.5", "Select Hawliad wedi’i Wrthod from Cyflwr listbox", "snapshot=Common_2.5.inf");
	...
	truclient_step("3", "Function SelectRandomCase", "snapshot=Common_3.inf");
	{
		...
		truclient_step("3.3", "Evaluate JavaScript code // Get all elements with...' + randomLink);", "snapshot=Common_3.3.inf");
		/* Pick a random case link */
		lr_start_transaction("060_SelectCase");
		truclient_step("3.5", "Navigate to randomLink", "snapshot=Common_3.5.inf");
