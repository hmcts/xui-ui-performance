...
Action()
{
	...
	truclient_step("7", "WelshTranslation", "snapshot=Action_7.inf");
	{
		...
		lr_start_transaction("065_Wait30Seconds");
		truclient_step("7.2", "Wait 30 seconds", "snapshot=Action_7.2.inf");
		lr_end_transaction("065_Wait30Seconds",0);
		lr_start_transaction("066_TabExists");
		truclient_step("7.3", "Wait until Dogfennau achos tab exists", "snapshot=Action_7.3.inf");
		lr_end_transaction("066_TabExists",0);
	}
